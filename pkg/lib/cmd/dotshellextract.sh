...
dotshellextract.main() {
	...
	generatedDir="${XDG_STATE_HOME:-$HOME/.local/state}/dotshellextract"
	currentDir="$BASALT_PACKAGE_PATH/pkg/lib/cmd"
	...
	find "$XDG_CONFIG_HOME/shell/modules/functions/" -ignore_readdir_race -type f -name "*.sh" \
			-exec sh -c "\"$currentDir/extract_functions.pl\" 'user' < \"\$0\"" {} \; >&6
	...
	util_print_autogen_info >&6
	find "$XDG_CONFIG_HOME/shell/modules/functions/" -ignore_readdir_race -type f -name "*.sh" \
			-exec sh -c "\"$currentDir/extract_functions.pl\" 'root' < \"\$0\"" {} \; >&6
	util_print_file "$profileDir/modules/util.sh" >&6
	exec 6<&-
